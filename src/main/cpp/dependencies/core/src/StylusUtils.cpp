...
{
	jclass classJFrame = env->FindClass("java/awt/Window");
	if (env->IsInstanceOf(window, classJFrame))
	{
		jlong windowId = GetJavaAwtWindowId(env, window);
    ...
    } else if (javaClassName.compare("javafx.stage.Stage") == 0) {
        windowId = GetJavaFx9WindowId(env, window);
//    } else if (javaClassName.compare("javax.swing.JFrame") == 0) {
//        windowId = GetJavaAwtWindowId(env, window);
    } else if (javaClassName.compare("java.lang.Long") == 0) {
